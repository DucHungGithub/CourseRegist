...
void ViewStudentInClass(NodeClass*& ClassInfo, Student*& StudentList, User*& UserList) {
    ...
    //int ListLen[8] = { 5,20,45,8,22,12,12,8 };
    //string ListTitle[7] = {"#", "Student ID", "Student Name", "Gender", "Date of birth", "GPA","GPAALL", "Option" };
    ...
    //Button stasticbutton = Button((GetWidth() - TableWidth) / 2 + TableWidth -1 , 0, 1, 1, "O");
    //stasticbutton.DrawButton();
    ...
    while(1) {
        ...
        while(!isLeftMousePressed(MousePosition));
        /*if (stasticbutton.isClick(MousePosition))
        {
            PrintALLStatistics(ClassInfo->listStudents);
            _getch();
            system("cls");
        }*/
        if(QuitButton.isClick(MousePosition)) return;
